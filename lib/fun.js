...
const glitext = async (teks1, teks2) => new Promise((resolve, reject) => {
    console.log('Creating glitch text picture...')
	axios.get(`https://api.vhtear.com/glitchtext?text1=${teks1}&text2=${teks2}&apikey=yourApikey`)
	.then((res) => {
		const logo = `https://api.vhtear.com/glitchtext?text1=${teks1}&text2=${teks2}&apikey=yourApikey`
		const textp = 'Ini'
		resolve({link: logo, text: textp})
	})
	.catch((err) => {
		reject(err)
	})

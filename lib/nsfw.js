...
/**
 * Get XVIDEOS video from URL.
 * @param {String} url 
 * @returns {Object}
 */
const xvdl = (url) => new Promise((resolve, reject) => {
    console.log(`Get XVIDEOS video from ${url}`)
    fetchJson(`http://hujanapi.xyz/api/xvideosdl?url=${url}&apikey=${config.hujanapi}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})

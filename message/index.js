...
function getDisplayName(pushname, verifiedName, formattedName) {
    return pushname || verifiedName || formattedName
}

function getChats(type, body, caption) {
    if (type === 'chat') {
        return body
    }
...
function getCommandBody(type, body, caption, prefix) {
    if (type === 'chat' && body.startsWith(prefix)) {
        return body
    }
    if ((type === 'image' || type === 'video') && caption && caption.startsWith(prefix)) {
        return caption
    }
    return ''
}

function getCommandFromBody(body) {
    return body.slice(1).trim().split(/ +/).shift().toLowerCase()
}
...
module.exports = msgHandler = async (bocchi = new Client(), message) => {
    try {
        ...
        const displayName = getDisplayName(pushname, verifiedName, formattedName)
        ...
        const chats = getChats(type, body, caption)
        body = getCommandBody(type, body, caption, prefix)
        const command = getCommandFromBody(body)
        const args = getArgsFromBody(body)
        const q = args.join(' ')
        ...
        if (isGroupMsg) {
            ...
            if (afk.checkAfkUser(sender.id, _afk) && !isCmd) {
                ...
                await bocchi.sendText(from, eng.afkDone(displayName))
            }
        }
        ...
        if (isCmd && (isBanned || isBlocked) && !isGroupMsg) return console.log(color('[BAN]', 'red'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(displayName))
        if (isCmd && (isBanned || isBlocked) && isGroupMsg) return console.log(color('[BAN]', 'red'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(displayName), 'in', color(name || formattedTitle))

        // Anti spam
        if (isCmd && msgFilter.isFiltered(from) && !isGroupMsg) return console.log(color('[SPAM]', 'red'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(displayName))
        if (isCmd && msgFilter.isFiltered(from) && isGroupMsg) return console.log(color('[SPAM]', 'red'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(displayName), 'in', color(name || formattedTitle))

        // Log
        if (isCmd && !isGroupMsg) {
            console.log(color('[CMD]'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(displayName))
            await bocchi.sendSeen(from)
        }
        if (isCmd && isGroupMsg) {
            console.log(color('[CMD]'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(displayName), 'in', color(name || formattedTitle))
            await bocchi.sendSeen(from)
        }
        ...
        switch (command) {
            ...
            case 'afk': // by Slavyan
                ...
                await bocchi.reply(from, eng.afkOn(displayName, reason), id)
                break
            ...
            case 'help':
                ...
                if (args[0] === '1') {
                    ...
                    await bocchi.sendText(from, eng.menu(jumlahUser, levelMenu, xpMenu, role, displayName, reqXpMenu, isPremium ? 'YES' : 'NO'))
                }
                break
            case 'rules':
            case 'rule':
            ...
            case 'report':
                ...
                if (lastReport !== undefined && cd - (Date.now() - lastReport) > 0) {
                    ...
                    if (isGroupMsg) {
                        ...
                        await bocchi.reply(from, eng.received(displayName), id)
                    } else {
                        await bocchi.sendText(ownerNumber, `*── 「 REPORT 」 ──*\n\n*From*: ${displayName}\n*ID*: ${sender.id}\n*Message*: ${q}`)
                        await bocchi.reply(from, eng.received(displayName), id)
                    }
            ...
            case 'join':
                ...
                if (isOwner) {
                    ...
                    await bocchi.sendText(checkInvite.id, `Hello! I was invited by ${displayName}`)
                } else {
            ...
            case 'setstat':
                ...
                break
            case 'mute':
                if (!isRegistered) return await bocchi.reply(from, eng.notRegistered(displayName), id)
                if (!isGroupMsg) return await bocchi.reply(from, eng.groupOnly(), id)
                if (!isGroupAdmins) return await bocchi.reply(from, eng.adminOnly(), id)
                if (ar[0] === 'enable') {
